...
export default function Files({ showToast }) {
    ...
    async function generateFromYoutube() {
        ...
        setLoadingYt(true);
        try {
            // FIX: backend /api/youtube expects { url, count } not { youtubeUrl }
    ...
    async function extractFromFile(file) {
        setLoading(true);
        try {
            // FIX: backend /api/flashcards expects { content } not { text } or { b64 }
            const content = file.text && file.text.trim().length > 10 ? file.text : null;
            if (!content) {
    ...
    async function generatePlanFromFile(file) {
        ...
        try {
            const content = file.text && file.text.trim().length > 10 ? file.text : null;
            if (!content) {
                showToast('Error: Could not read text from this file.');
                return;
            }
            ...
            if (Array.isArray(result)) {
                ...
                if (Array.isArray(planObj)) {
                    ...
                    raw = Array.isArray(planObj.plan) ? planObj.plan
                        : (Object.values(planObj).find(v => Array.isArray(v)) || []);
                }
            }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div
                style={{ ...card({ border: `2px dashed ${dragOver ? C.a : C.b}`, background: dragOver ? C.aD : C.s, textAlign: 'center', padding: 32, cursor: 'pointer', transition: 'all 0.2s' }) }}
            ...
            <div style={card()}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    />
                    <button onClick={generateFromYoutube} disabled={loadingYt || !ytUrl.trim()} style={btn('p')}>
                        {loadingYt ? '...' : 'Generate'}
                    </button>
                </div>
            </div>
            ...
            {files.length > 0 && (
                <div style={card()}>
                    <h3 style={{ color: C.tx, fontSize: 15, fontWeight: 700, marginBottom: 12 }}>
                        ...
                        {files.map(f => (
                            <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.s2, borderRadius: 8, padding: '10px 12px' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ color: C.tx, fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        {f.name}
                                    </div>
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    style={btn('p', { padding: '6px 12px', fontSize: 12 })}
                                    title="Generate flashcards"
                                >
                                    {loading ? '...' : '🃏 Cards'}
                                </button>
                                <button
                                    onClick={() => generatePlanFromFile(f)}
                                    disabled={loading}
                                    style={btn('p', { padding: '6px 12px', fontSize: 12, background: '#6366f1' })}
                                ...
                                <button
                                    ...
                                    style={btn('d', { padding: '6px 10px', fontSize: 12 })}
                                >
                                    ✕
                                </button>
                            </div>
            ...
            {files.length === 0 && (
                <div style={{ textAlign: 'center', color: C.mu, padding: 32 }}>
                    No files uploaded yet. Drop some study material above!
                </div>
            )}
        </div>
    );
