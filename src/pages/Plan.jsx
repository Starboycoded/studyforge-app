...
export default function Plan({ showToast }) {
    ...
    async function generate() {
        ...
        if (sourceMode === 'file') {
            ...
            if (!file.text || file.text.trim().length < 10) { showToast('No text extracted from this file. Remove and re-upload it.'); return; }
            content = file.text; subject = file.name.replace(/\.[^.]+$/, '');
        } else {
            if (!topic.trim()) return;
            content = topic.trim(); subject = topic.trim();
        }
        setLoading(true);
        try {
            const result = await apiFetch('/api/plan', { content, subject, hoursPerDay: Number(hoursPerDay), examDate: `${Number(days)} days from now` });
            let raw = [], meta = null;
            if (Array.isArray(result)) { raw = result; }
                ...
                if (Array.isArray(planObj)) { raw = planObj; }
                    ...
                    if (Array.isArray(planObj.plan)) { raw = planObj.plan; meta = { totalDays: planObj.totalDays, tips: planObj.tips, summary: planObj.summary }; }
                    else { const found = Object.values(planObj).find(v => Array.isArray(v)); raw = found || []; }
                }
            } else if (Array.isArray(result?.data)) { raw = result.data; }
            if (!raw.length) throw new Error('No plan returned from server');
            setPlan(raw); setPlanMeta(meta); LS.set('sf_plan', raw); LS.set('sf_plan_meta', meta);
            showToast('Study plan generated!');
        } catch (ex) { showToast('Error: ' + ex.message); } finally { setLoading(false); }
    }

    function clearPlan() { setPlan([]); setPlanMeta(null); LS.set('sf_plan', []); LS.set('sf_plan_meta', null); }

    const byDay = plan.reduce((acc, s) => {
        const dayKey = s.day !== undefined ? `Day ${s.day}` : (s.date || 'Day 1');
        if (!acc[dayKey]) acc[dayKey] = [];
        acc[dayKey].push(s); return acc;
    }, {});
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {sourceMode === 'topic' && (
                        <div>
                            ...
                            <input style={inp()} type="text" placeholder="e.g. Machine Learning, Calculus, History..." value={topic} onChange={e => setTopic(e.target.value)} onKeyDown={e => e.key === 'Enter' && generate()} />
                        </div>
                    )}
                    {sourceMode === 'file' && (
                        <div>
