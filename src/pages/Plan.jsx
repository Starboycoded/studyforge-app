...
export default function Plan({ showToast }) {
    ...
    async function generate() {
        ...
        setLoading(true);
        try {
            // FIX: backend expects { content, subject, hoursPerDay, examDate }
            ...
            // FIX: backend returns { success, plan: { totalDays, plan: [...], tips, summary } }
            let raw = [];
            let meta = null;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {planMeta && (
                <div style={{ ...card(), borderLeft: `3px solid ${C.a}` }}>
                    {planMeta.summary && (
                        <p style={{ color: C.tx, fontSize: 13, marginBottom: planMeta.tips?.length ? 10 : 0 }}>
                            {planMeta.summary}
                        </p>
                    )}
                    {planMeta.tips?.length > 0 && (
                        <div>
                            <div style={{ color: C.mu, fontSize: 12, fontWeight: 700, marginBottom: 6 }}>💡 Tips</div>
                            {planMeta.tips.map((tip, i) => (
                                <div key={i} style={{ color: C.mu, fontSize: 12, marginBottom: 4 }}>• {tip}</div>
                            ))}
                        </div>
                    )}
                </div>
            ...
            {plan.length > 0 && (
                <div style={card()}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                        <h3 style={{ color: C.tx, fontSize: 15, fontWeight: 700 }}>
                            Your Plan ({plan.length} days)
                        </h3>
                        <button onClick={clearPlan} style={btn('d', { padding: '6px 12px', fontSize: 12 })}>
                            Clear
                        </button>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        {Object.entries(byDay).map(([dayLabel, sessions]) => (
                            <div key={dayLabel}>
                                <div style={{
                                    color: C.a, fontSize: 13, fontWeight: 700,
                                ...
                                {sessions.map((s, i) => (
                                    <div key={i} style={{
                                        ...
                                        marginBottom: 6, borderLeft: `3px solid ${C.a}`,
                                    }}>
                                        <div style={{ color: C.tx, fontSize: 13, fontWeight: 600 }}>
                                            {s.focus || s.topic || s.title || s.subject || `Session ${i + 1}`}
                                        </div>
                                        {(s.duration || s.time || s.hours) && (
                                            <div style={{ color: C.mu, fontSize: 12, marginTop: 2 }}>
                                                ⏱ {s.duration || s.time || s.hours}
                                            </div>
                                        )}
                                        {s.topics?.length > 0 && (
                                            <div style={{ color: C.mu, fontSize: 12, marginTop: 4 }}>
                                                📖 {Array.isArray(s.topics) ? s.topics.join(', ') : s.topics}
                                            </div>
                                        )}
                                        {s.tasks?.length > 0 && (
                                            <div style={{ color: C.mu, fontSize: 12, marginTop: 4 }}>
                                                ✅ {Array.isArray(s.tasks) ? s.tasks.join(' · ') : s.tasks}
                                            </div>
                                        )}
                                        {s.description && (
                                            <div style={{ color: C.mu, fontSize: 12, marginTop: 4 }}>
            ...
            {plan.length === 0 && !loading && (
                <div style={{ textAlign: 'center', color: C.mu, padding: 32 }}>
                    No plan yet. Enter a topic above to generate your personalised study schedule!
                </div>
            )}
        </div>
    );
