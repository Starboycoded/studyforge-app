    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {totalCards > 0 && (
                <div style={card()}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                        <h3 style={{ color: C.tx, fontSize: 15, fontWeight: 700 }}>Card Mastery</h3>
                        <span style={{ color: C.gr, fontWeight: 700 }}>{masteryPct}%</span>
                    </div>
                    <div style={{ background: C.s2, borderRadius: 6, height: 10, overflow: 'hidden' }}>
                        <div style={{ width: `${masteryPct}%`, height: '100%', background: `linear-gradient(to right, ${C.a}, ${C.gr})`, borderRadius: 6, transition: 'width 0.5s ease' }} />
                    </div>
                ...
                {quizHistory.length > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {[...quizHistory].reverse().map((q, i) => {
                            ...
                            return (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.s2, borderRadius: 8, padding: '10px 12px' }}>
                                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: pct >= 70 ? C.gr + '22' : C.re + '22', border: `2px solid ${pct >= 70 ? C.gr : C.re}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: pct >= 70 ? C.gr : C.re, fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                                        {pct}%
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ color: C.tx, fontSize: 13, fontWeight: 600 }}>{q.topic}</div>
                                        <div style={{ color: C.mu, fontSize: 11 }}>{q.score}/{q.total} correct · {new Date(q.date).toLocaleDateString()}</div>
                                    </div>
                                </div>
                            );
                        })}
            ...
            {cards.length > 0 && (() => {
                ...
                return (
                    <div style={card()}>
                        <h3 style={{ color: C.tx, fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Cards by Course</h3>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                ...
                                return (
                                    <div key={course}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                            <span style={{ color: C.tx, fontSize: 13 }}>{course}</span>
                                            <span style={{ color: C.mu, fontSize: 12 }}>{count} cards</span>
                                        </div>
                                        <div style={{ background: C.s2, borderRadius: 4, height: 6 }}>
                                            <div style={{ width: `${pct}%`, height: '100%', background: C.a, borderRadius: 4 }} />
                                        </div>
                                    </div>
                                );
                            })}
